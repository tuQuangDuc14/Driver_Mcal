...
uint8_t MCL_Start(MCL_Handle_t *pMCLHandle, uint32_t SrcAddress, uint32_t DstAddress, uint32_t DataLength)
{
    ...
    if(DMA_STATE_READY == pMCLHandle->State)
    {
        ...
        /* Enable Peripheral to start transfer data*/
        pMCLHandle->pSTREAMx->CR |= DMA_SxCR_EN;
    }
    else
    { 
...
uint8_t MCL_WaitForTransfer(MCL_Handle_t *pMCLHandle)
{
    ...
    Mask_CpltLevel = DMA_FLAG_TCIF0_4 << pMCLHandle->MCLConfig.StreamIndex; /*set gtri 1 cho stream mà cần check*/
    TmpISR = Register->ISR; /*get gitri của thanh ghi isr*/
    if(((TmpISR & Mask_CpltLevel) == RESET) && ((pMCLHandle->ErrorCode & DMA_ERROR_TE) == RESET)) /*ktra neu flag TCIF k set bao loi */
    {
        /* Get the ISR register value */
        TmpISR = Register->ISR;

        if((TmpISR & (DMA_FLAG_TEIF0_4 << pMCLHandle->MCLConfig.StreamIndex)) != RESET) /* Stream x transfer error interrupt flag*/
        {
            /* Update error code */
        ...
        if((TmpISR & (DMA_FLAG_FEIF0_4 << pMCLHandle->MCLConfig.StreamIndex)) != RESET)/*Stream x FIFO error interrupt flag*/
        {
            /* Update error code */
        ...
        if((TmpISR & (DMA_FLAG_DMEIF0_4 << pMCLHandle->MCLConfig.StreamIndex)) != RESET)/*Stream x direct mode error interrupt flag*/
        {
            /* Update error code */
...
uint8_t MCL_Start_IT(MCL_Handle_t *pMCLHandle, uint32_t SrcAddress, uint32_t DstAddress, uint32_t DataLength)
{
    ...
    if(DMA_STATE_READY == pMCLHandle->State)
    {
        ...
        /* Clear all interrupt flags at correct offset within the register */
        Register->IFCR = 0x3FU << pMCLHandle->MCLConfig.StreamIndex;
        /* Enable Common interrupts trước khi start chennel*/
