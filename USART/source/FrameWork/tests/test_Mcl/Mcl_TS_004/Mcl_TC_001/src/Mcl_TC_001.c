...
void Mcl_TC_001(void)
{
	...
	DMAUart.pDMAx = DMA1;
	DMAUart.pSTREAMx = DMA1_Stream5; 
	DMAUart.MCLConfig.Channel = DMA_CHANNEL_4 ;
	DMAUart.MCLConfig.Direction = DMA_PERIPH_TO_MEMORY;
	...
	MCL_IRQPriorityConfig(IRQ_NO_DMA1_SREAM_5,NVIC_IRQ_PRI15);
	MCL_IRQInterruptConfig(IRQ_NO_DMA1_SREAM_5,ENABLE);
	...
	while(1)
	{
		// MCL_Start_IT(&DMAUart,(uint32_t)&USART2_Handle.pUSARTx->DR,(uint32_t)Tx_buf,16);
		MCL_Start_IT(&DMAUart,(uint32_t)&USART2_Handle.pUSARTx->DR,(uint32_t)DEST_ADDRESS,16);

		/* Send UART2_TX DMA Request to DMA1 Controler */
		USART2_Handle.pUSARTx->CR3 =  ( 1 << USART_CR3_DMAR) ;
	}
